<template>
    <template v-if="userStore.profileCompleted">
    ...
    <template v-else>
        <div class="flex flex-col gap-6 items-center justify-center text-center">
            <p class="mainHeading max-w-xl">Продолжите заполнения профиля или зарегистрируйтесь, чтобы пользоваться всеми функциями сайта</p>
            <NuxtLink to="/profile" class="mx-auto px-4 py-2 border border-sky-500 text-sky-500 rounded-full w-[160px] text-center transition-all duration-500 hover:text-white hover:bg-sky-500">Продолжить</NuxtLink>
        </div>
    </template>
</template>
